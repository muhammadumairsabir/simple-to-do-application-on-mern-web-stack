...
The whole purpose of adding the proxy configuration in number 3 above is to make it possible to access the application directly from the browser by simply calling the server url like<span style="color:red"></span> rather than always including the entire path like http://localhost:5000/api/todos
