...
Create a GET request to your API on http://<PublicIP-or-PublicDNS>:5000/api/todos<span style="color:red">:</span>. This request retrieves all existing records from out To-do application (backend requests these records from the database and sends it us back as a response to GET request).
